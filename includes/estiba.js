...
(function () {
  ...
  function _esTurnoHoy() {
    return state.turnoFecha === new Date().toISOString().slice(0, 10);
  }
  ...
  async function registrarCambio() {
    const p = state.personal.find(x => x.id === state.selectedId);
    if (!p) return;
    const prev  = { funcion: p.funcion, ubicacion: p.ubicacion, horario: p.horario || '', estado: p.estado, radio: p.radio ? 1 : 0 };
    const nuevo = { funcion: $('dwFuncion').value, ubicacion: $('dwUbicacion').value, horario: ($('dwHorario') ? $('dwHorario').value : (p.horario || '')), estado: $('dwEstado').value || 'activo', radio: $('dwRadio').value === '1' ? 1 : 0 };
    ...
    try {
      await apiPost('api/update_asignacion.php', {
        tpId: p.tpId, funcion: p.funcion, ubicacion: p.ubicacion, horario: p.horario, estado: p.estado, radio: p.radio
      });
      ...
      if (state.selectedId === p.id) { $('dwFuncion').value = p.funcion; $('dwUbicacion').value = p.ubicacion; if ($('dwHorario')) $('dwHorario').value = p.horario || ''; $('dwEstado').value = p.estado; $('dwRadio').value = p.radio ? '1' : '0'; }
      renderGrid(); renderKpis(); renderDrawerAlert(p);
  ...
  async function confirmAddPersonal() {
    ...
    try {
      const r = await apiPost('api/add_personal_turno_masivo.php', {
        turnoId: state.turnoId, ubicacion, horario, colaboradores
      });
      if (r.agregados && r.agregados.length) {
        r.agregados.forEach(p => {
          state.personal.push({ ...p, bitacora: [] });
          pushAccion('alta', `Agregado al turno · ${p.funcion}${ubicacion ? ' @ ' + ubicacion : ''}`, p.id, p.nombre);
        });
