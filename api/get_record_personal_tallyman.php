...
$propuestas=rpt_query($conn,"SELECT id,detalle,viabilidad,impacto,puntaje_comentario,puntaje_por,puntaje_at,created_at FROM sugerencias_tallyman WHERE colaborador_id=? AND canal='propuesta' AND viabilidad IS NOT NULL ORDER BY puntaje_at DESC,id DESC",$id);
...
$evaluacionesDesempeno=rpt_query($conn,"SELECT periodo,puntaje_total,aspectos_mejora,coordinador_nombre,fecha_evaluacion FROM evades_evaluaciones WHERE colaborador_id=? ORDER BY periodo DESC,fecha_evaluacion DESC,id DESC",$id);
$charlas=rpt_query($conn,"SELECT a.id,a.tema,a.tipo_reunion,a.fecha,a.hora,a.lugar,a.capacitador,p.estado FROM asistencias_participantes p JOIN asistencias_preoperativas a ON a.id=p.asistencia_id WHERE p.colaborador_id=? AND p.estado IN ('asistio','tardanza','falta') ORDER BY a.fecha DESC,a.hora DESC,a.id DESC",$id);
...
$asistenciasCharlas=count(array_filter($charlas,fn($x)=>$x['estado']==='asistio'));
$faltasCharlas=count(array_filter($charlas,fn($x)=>$x['estado']==='falta'));
echo json_encode(['success'=>true,'perfil'=>$perfil,'resumen'=>['incidencias'=>count($incidencias),'sanciones'=>count($sanciones),'propuestas'=>count($propuestas),'reconocimientos_aprobados'=>$aprobados,'evaluaciones_desempeno'=>count($evaluacionesDesempeno),'charlas'=>$asistenciasCharlas + $tardanzasCharlas,'asistencias_charlas'=>$asistenciasCharlas,'tardanzas_charlas'=>$tardanzasCharlas,'faltas_charlas'=>$faltasCharlas,'capacitaciones'=>count($capacitaciones)],'historial'=>compact('incidencias','sanciones','propuestas','reconocimientos','evaluacionesDesempeno','charlas','capacitaciones')],JSON_UNESCAPED_UNICODE);
