...
export const NavesModel = {
  ...
  async actualizar(id, { nombre, muelle, tipo_nave_id, actividad_id, eta, etb, etd, estado }) {
    ...
    // Propagar la edición de la nave a las actividades (registros) hechas con ella,
    // para que reflejen el muelle y la actividad vigentes de la nave (solo BERTH).
    if (muelle) {
      await pool.query(
    ...
    if (actividad_id) {
      await pool.query(
        `UPDATE tallyman_registros SET actividad_id = ? WHERE nave_id = ? AND ubicacion_tipo = 'BERTH'`,
        [actividad_id, id],
      );
    }
  ...
  // Actualiza SOLO el tipo de nave (sin tocar el resto de campos base). Usado por
  // el registro tallyman de muelle para propagar a Operaciones el "TIPO DE NAVE"
  // que el coordinador ajuste desde la actividad.
  async actualizarTipo(id, tipoNaveId) {
    await pool.query('UPDATE naves SET tipo_nave_id = ? WHERE id = ?', [tipoNaveId, id]);
    return this.obtenerPorId(id);
  },

  // Actualiza SOLO el muelle y/o la actividad de la nave (sin tocar otros campos
  // ni los registros de tallyman). Usado por el registro tallyman de muelle para
  // propagar a Operaciones el muelle/actividad que el coordinador ajuste al editar.
  // Cada campo se aplica solo si viene definido (undefined = no tocar).
  async actualizarMuelleActividad(id, { muelle, actividad_id }) {
    const sets = [], params = [];
    if (muelle !== undefined)       { sets.push('muelle = ?');       params.push(muelle ?? null); }
    if (actividad_id !== undefined) { sets.push('actividad_id = ?'); params.push(actividad_id ?? null); }
    if (sets.length) {
      params.push(id);
  ...
  async actualizarOperativo(naveId, { eta, etb, etd, estado } = {}) {
    const sets = [];
    const params = [];
